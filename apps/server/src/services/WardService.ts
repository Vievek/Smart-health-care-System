...
export class WardService implements IService<IWard> {
  ...
  async allocateBed(bedId: string, patientId: string): Promise<IBed | null> {
    console.log(`Allocating bed ${bedId} (patient details redacted)`)
  ...
  async transferPatient(currentBedId: string, newBedId: string): Promise<IBed | null> {
    console.log(`Transferring patient (details redacted) from bed ${currentBedId} to ${newBedId}`)
  ...
  async dischargePatient(bedId: string): Promise<IBed | null> {
    console.log(`Discharging patient (details redacted) from bed ${bedId}`)
