...
export class MedicalRecordService implements IService<IMedicalRecord> {
  constructor(private medicalRecordRepo: MedicalRecordRepository = new MedicalRecordRepository()) {}
  ...
  async getPatientRecords(patientId: string, userRole: string): Promise<IMedicalRecord[]> {
    console.log(`Getting records (PII redacted)`)

    // For patients, only return their own records
    if (userRole === 'patient') {
      const records = await this.medicalRecordRepo.findByPatientId(patientId)
      console.log(`Found ${records.length} records (PII redacted)`)
      return records
    }
  ...
  async getPrescriptionsByPatient(patientId: string): Promise<IMedicalRecord[]> {
    console.log(`Getting prescriptions (PII redacted)`)
    try {
      // FIXED: Use the correct repository method
      const prescriptions = await this.medicalRecordRepo.findPrescriptionsByPatient(patientId)
      console.log(`Found ${prescriptions.length} prescriptions (PII redacted)`)
      return prescriptions
    } catch (error) {
      console.error(`Error getting prescriptions (PII redacted):`, error)
      throw error
    }
  }
  ...
  async generateRecordPDF(recordId: string): Promise<Buffer> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        doc.text(`Created Date: ${new Date(record.createdDate).toLocaleDateString()}`)
